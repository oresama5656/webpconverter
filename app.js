...
function openBrowser(url) {
    ...
    console.log(`ブラウザを起動中: ${url}`);
    exec(command, (error) => {
        if (error) {
            console.log('ブラウザの自動起動に失敗しました。手動でアクセスしてください。');
...
const server = app.listen(PORT, () => {
    console.log('🚀 WebP変換ツール（リサイズ対応）が起動しました');
    console.log(`📍 URL: http://localhost:${PORT}`);
    console.log('📂 ブラウザでアクセスして画像をアップロードしてください');
    console.log('終了するには Ctrl+C を押してください');
    console.log('');
    
    // 常にブラウザを自動起動（pkg実行時も含む）
    setTimeout(() => {
    ...
    if (err.code === 'EADDRINUSE') {
        ...
        console.error('他のアプリケーションを終了するか、しばらく待ってから再試行してください。');
    }
    console.log('何かキーを押して終了してください...');
